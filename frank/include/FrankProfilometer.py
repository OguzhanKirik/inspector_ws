...
class newProfilometer():
    ...
    DELTA_T = None  #delay between trigger and pose
    RequestSave = False
    RequestStopSave = False
    EnableSave = False
    AlreadySaved = False
    firstDense = True
    secondDense = False
    ...
    R_EZ = math.radians(0.839)#(0.67525)
    R_EY = math.radians(-0.09267)#(-0.10451)
    R_EX = math.radians(179.9984)#(179.8448)
    ProfilometerCoordSystem = FrankUtilities.Euler2RotMatrix([R_EZ, R_EY, R_EX])
    ProfilometerOffset =[50.8, -14.5, -73.7396]#[5.07428104e+01, -1.22410951e+01, -7.29618616e+01]
    ...
    def InitializeProfilometer(self, ProfilesPerFrame : int, xScale : float):
        ...
        result = cxc.cx_setParam(self.device, "ExposureTime", 5000)

        result = cxc.cx_setParam(self.device, "AoiHeight", 2048) # 3072
        result = cxc.cx_setParam(self.device, "AoiOffsetY", 1024) # 0
        
        #result = cxc.cx_setParam(self.device, "CameraMode", "Threshold")
        result = cxc.cx_setParam(self.device, "AoiThreshold", 530)
        ...
        result = cxc.cx_setParam(self.device, "LightBrightness", 20.0)
    ...
    def Acquire_trigger(self,trig_starter, statepub,SavePath='', SaveToFile=False, RobotLinSpeed = 100, xScale = 0.125, ProfilesPerFrame =100):    ###### xScale=0.2 ######## profilesperframe = 100
        '''
        ...
        self.AcquisitionStarted = True
        print("SAVING ALL FILES IN : ", self.Filepath)
        Profile = list()
        RangeImg = list()
        NN = 0
        self.RequestedStop = False
        ...
        while True:
            ...
            result = c3d.cx_3d_range2rectifiedC(self.calib, cxRangeImg, zMap, c3d.CX_3D_METRIC_MARK_Z_INVALID_DATA
                                                | c3d.CX_3D_METRIC_FILL_HOLES)
            
            
            #cxc.cx_queueBuffer(buffer)
            #######################################
            ### Saving data just when requested ###
            #######################################

            if self.EnableSave == True:
                if self.num_profile == 0:
                    #opening the file just the first time and then appending to it
                    file = open(SavePath+"/PointCloud_"+str(self.id_acqu)+'.txt',"w")
                    self.num_profile=1
                ...
                #     # if np.sum([len(Profile) - 1]) == 0:
                #     #    # print("Profile = 0, N = " + str(NN) + ", i = " + str(i))
                # for i in range(0, len(Profile)):
                    stringList = '\t'.join([str(item) for item in zMap.data[i]]) + "\n"
                    file.write(stringList)
                if prof == 0:
                    Profile0 = zMap.data[0]
                    print(len(Profile0))
                    stamp_buff1 = stamp_buff
                    prof+=1
      
            cxc.cx_queueBuffer(buffer)
            if self.RequestSave == True:
                print("arrivata la richiesta di salvataggio!")
                self.EnableSave = True
                self.RequestSave = False
            if self.RequestStopSave==True:
                ### finishing to save all the data in a thread
                print("arrivata la richiesta di fine salvataggio!")

                numX = len(Profile0)* xScale
                XX = np.arange(-numX / 2, numX / 2, xScale)
                stringList = '\t'.join([str(item) for item in XX]) + "\n"
                file.write(stringList)
                file.close()    # closing the profile data file
                print("print Saving Robot Poses: ")
                file = open(SavePath+"/egm_poses"+str(self.id_acqu)+".txt","w")
                np.savetxt(file,self.pose_robot)
                file.close()
                print("print Saving trigger time stamp")
                file = open(SavePath+"/trigger_stamp"+str(self.id_acqu)+".txt","w")
                file.write(str(stamp_buff1.to_sec()-1)+" "+ str(stamp_buff1.to_nsec()-1000000000))
                file.close()
                self.pose_robot = []
                self.counter_pose = 0
                self.Append == False
                prof=0
                if self.firstDense == True:
                    
                    self.firstDense = False
                    self.secondDense = True
                    self.firstscanid = self.id_acqu
                elif self.secondDense == True:
                    self.main_thread = threading.Thread(target = self.CalibrateTimestamp_threader, args = (SavePath,statepub, self.id_acqu,self.firstscanid))
                    self.main_thread.start()
                    self.secondDense = False
                else:
                    self.id_list.append(self.id_acqu)
                    # self.other_threads = threading.Thread(target =self.SyncronizePointclouds_threader, args = (SavePath,statepub,self.id_acqu))
                    # self.other_threads.start()

                self.RequestStopSave = False
                self.EnableSave = False
            if self.RequestedStop:   #### request of all the acquisition to stop
                break
        ...
        t2 = threading.Thread(target =self.SyncronizePointclouds_threader, args = (SavePath,statepub,self.id_list))
        t2.start()
        
        return #[XX,ZZ]# [XX, YY, ZZ]

    def SyncronizePointclouds_threader(self,filepath,statepub,num_list):
        # while self.DELTA_T == None:
        #     #print("Waiting for timestamp calibration")
        #     time.sleep(10)
        print("___Waiting for timestamp calibration to finish___")
        self.main_thread.join()
        print("___Timestamp Calibration finished!___")
        # print("___optimizing x y offsets___")
        # #first syncronize the 
        # print(self.DELTA_T)
        # syncronizer.sync_function(filepath,statepub,-1, self.DELTA_T, self)
        # syncronizer.sync_function(filepath,statepub,-2, self.DELTA_T, self)
        # ox, oy = syncronizer.CalibrateXY(filepath, -2, -1) 
        # oz = self.ProfilometerOffset[2]
        # self.ProfilometerOffset = [ox,oy,oz]
        # syncronizer.sync_function(filepath,statepub,-1, self.DELTA_T, self)
        # syncronizer.sync_function(filepath,statepub,-2, self.DELTA_T, self)
        t_ls = []
        for num in num_list:
            t_ls.append(threading.Thread(target =self.Syncronize_one_Pointcloud_threader, args = (filepath,statepub,num)))
        i=0
        for t in t_ls:
            t.start()
            time.sleep(3)
        ...
        for t in t_ls:
            t.join()
        print("FINISHED ALL POINTCLOUD")
        self.id_list = [] 
        self.DELTA_T = None

    def Syncronize_one_Pointcloud_threader(self,filepath,statepub,num):
        syncronizer.sync_function(filepath,statepub,num, self.DELTA_T,self)

    def CalibrateTimestamp_threader(self, filepath, statepub,num2,num1):
        self.DELTA_T = syncronizer.CalibrateTimestamp(filepath,statepub, num2, num1)
    ...
    def ReadPointCloudInputTextFile(self, filePath):
        '''
    ...
    def BuildPointCloud_RobotCoordinate2(self,Num, Coord, Quat, XX, YY, ZZ, SavePath,ProfilometerCoordSystem,ProfilometerOffset):
        ...
        #PROPOSED NEW ALGOTITHM (working)
        profiRT = np.array(ProfilometerCoordSystem)
        #print(profiRT)#############################################
        profiRT = np.transpose(profiRT)
        ...
        for i in range(0, min(NCoord, NY)): #(0, NY): #THIS MUST BE SOLVED WITH PROPER SYNC BETWEEN PROFILOMETER AND COORD
            ...
            points[i*NX:(i+1)*NX, 1] = np.array(YY[i]) * np.ones((NX))
            points[i*NX:(i+1)*NX, 2] = np.array(ZZ[i])
            punti_prof.append(copy.deepcopy(points[i*NX:(i+1)*NX,:]))
            points[i*NX:(i+1)*NX, :] = points[i*NX:(i+1)*NX, :].dot(profiRT) + zero + offset
            RT = FrankUtilities.Quaternion2RotMatrix(Quat[i]) #Rotation matrix of the Tool
            RT = np.transpose(np.array(RT))
            cTemp = (np.array(Coord[i])).reshape((1,3))
            points[i*NX:(i+1)*NX, :] = points[i*NX:(i+1)*NX, :].dot(RT) + cTemp

            for j in range(len(points[i*NX:(i+1)*NX, 0])):
                coords.append(Coord[i])
                quats.append(Quat[i])
            pass
        print(len(coords))
        print(len(quats))
        punti_prof = np.concatenate(punti_prof, axis = 0)
        print(len(punti_prof))

        #bol = ~np.isnan(points[:,2])
        #points = points[bol,:]
        bol = np.isnan(punti_prof[:,2])
        punti_prof[bol,:] = [-1000, -1000, -1000] # added in order not to lose points
        bol = np.isnan(points[:,2])
        points[bol,:] = [-1000, -1000, -1000]
        if SavePath!='':
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(points)
            o3d.io.write_point_cloud(SavePath, pcd, write_ascii=False, compressed=True, print_progress=False)
        return points, punti_prof, coords, quats
