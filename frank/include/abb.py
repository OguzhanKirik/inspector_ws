...
class Robot:
    ...
    def get_cartesian_logger(self):
        ...
        r = [[0,0,0], [0,0,0,0]]
        r[0][0] = float(txt[Id0 + 4])
        r[0][1] = float(txt[Id0 + 5])
        r[0][2] = float(txt[Id0 + 6])
        
        r[1][0] = float(txt[Id0 + 7])
        r[1][1] = float(txt[Id0 + 8])
        r[1][2] = float(txt[Id0 + 9])
        r[1][3] = float(txt[Id0 + 10])
    ...
    def start_stop_profile(self, startstop, sampletime):
        '''
        startstop - 1 to start acquisition and 0 to stop acquisition
        sampletime - (float) in seconds
        '''
        msg = "37 " + str(startstop) + " " + str(sampletime) + " #"
        self.send(msg)
        return
        
    def change_singularity_interp(self, interpolationType):
        '''
        interpolationType:
            0 -> Off
            1 -> LockAxis4
            2 -> Wrist
        '''
        msg = "80 " + str(interpolationType) + " #"
        self.send(msg)
        return
        
    def change_configuration_linear_monitoring(self, monitoringType):
        '''
        monitoringType:
            0 -> Off
            1 -> On
        '''
        msg = "81 " + str(monitoringType) + " #"
        self.send(msg)
        return
    ...
    def format_pose(self, pose):
        ...
        for cartesian in pose[0]:
            msg += str(round(float(cartesian * self.scale_linear),1)) + " " 
        for quaternion in pose[1]:
            msg += str(round(float(quaternion),2)) + " " 
        msg += "#" 
        return msg       
